...
def build_items(sources, window_hours: int):
    items = []
    for s in sources:
        feed = feedparser.parse(s["rss"])
        for e in feed.entries[:60]:
...
def write_feed(filename: str, window_hours: int, items):
    now = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    out = {"generated_at_utc": now, "window_hours": window_hours, "items": items}
...
def main():
    ...
    # Genera cada ventana
    for filename, hours in windows.items():
        items = build_items(sources, hours)
        write_feed(filename, hours, items)

    # Compatibilidad: latest.json = 6h
    with open("latest_6h.json", "r", encoding="utf-8") as f:
        latest_6h = json.load(f)
    with open("latest.json", "w", encoding="utf-8") as f:
        json.dump(latest_6h, f, ensure_ascii=False, indent=2)
